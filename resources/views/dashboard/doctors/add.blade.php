...
@section('content')
    <div class="app-content content">
        ...
        <div class="content-wrapper">
            <x-dashboard.layouts.breadcrumb now="{{__('dashboard.add_doctor')}}">
                <li class="breadcrumb-item"><a href="{{route('admin.doctors.index')}}">{{__('dashboard.doctors_list')}}</a></li>
            </x-dashboard.layouts.breadcrumb>
            <div class="col-12 mt-3">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">{{__('dashboard.add_doctor')}}</h4>
                    </div>
                    <div class="card-content">
                        <div class="card-body">
                            <form class="form form-vertical" method="POST" action="{{route('admin.doctors.store')}}" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    ...
                                    <div class="form-group col-sm-3">
                                            <label style="width: 100%" for="specilizations">{{__('dashboard.specializations')}}
                                                <select multiple class="form-control select2 specilizations" name="specilizations[]">
                                                    @foreach($specialzations as $value)
