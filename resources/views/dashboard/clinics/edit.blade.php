...
@section('content')
    <div class="app-content content">
        ...
        <div class="content-wrapper">
            <x-dashboard.layouts.breadcrumb now="{{__('dashboard.edit_clinic')}}">
                <li class="breadcrumb-item"><a href="{{route('admin.clinics.index')}}">{{__('dashboard.clinics_list')}}</a></li>
            </x-dashboard.layouts.breadcrumb>
            <div class="col-12 mt-3">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">{{__('dashboard.edit_clinic')}}</h4>
                    </div>
                    <div class="card-content">
                        <div class="card-body">
                            <form class="form form-vertical" method="POST" action="{{route('admin.clinics.update',$clinic->id)}}" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <div class="row">
                                        ...
                                        <label style="width: 100%" for="doctor">{{__('dashboard.doctor')}}
                                            <select class="form-control select2 doctor" name="doctor">
                                                ...
                                                @foreach($doctors as $value)
                                                    <option @if(in_array($clinic->doctor_id,$doctors->pluck('id')->toArray())) selected @endif value="{{$value->id}}">{{$value->name}}</option>
                                                @endforeach
                                            </select>
                                            @error('doctor')
                                            <span style="font-size: 12px;" class="text-danger">{{$message}}</span>
                                    ...
                                    <div class="form-group col-sm-3">
                                        <div class="custom-control custom-switch custom-switch-danger mr-2 mb-1">
                                            ...
                                            <input @if($clinic->is_blocked == 1) checked @endif type="checkbox" class="custom-control-input" id="blocked_switch" name="is_blocked">
